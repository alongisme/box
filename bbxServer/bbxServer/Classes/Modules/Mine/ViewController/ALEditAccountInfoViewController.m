...
@property (nonatomic, strong) ALUpdateMyInfoApi *updateMyInfoApi;
@end
...
- (void)saveAction {
    ...
    if([self.nickNameView.textString nickNameVerify]) {
        _updateMyInfoApi = [[ALUpdateMyInfoApi alloc] initWithUpdateMyInfoApi:nil nickName:self.nickNameView.textString];
        
        [_updateMyInfoApi ALHudStartWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest * _Nonnull request) {
            [ALKeyWindow showHudSuccess:@"修改昵称成功～"];
            AL_MyAppDelegate.userModel.userInfoModel.nickName = weakSelf.nickNameView.textString;
...
- (ALShadowView *)nickNameView {
    if(!_nickNameView) {
        ...
        _nickNameView.textString = _nickNameView.nickName;
