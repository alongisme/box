...
- (void)addRefresh {
    ...
    if(self.shouldPullDownLoadMore) {
        ...
        } else if(_tableStyle == ALTableStyleMoney) {
            [footer setTitle:@"没有更多余额了～" forState:MJRefreshStateNoMoreData];
        }
...
- (BOOL)dataSourceIsEmpty {
    ...
    for (NSArray *array in self.dataSource) {
        if(array.count > 0) {
            isempty = NO;
            if(_tableStyle == ALTableStyleMyMessage || _tableStyle == ALTableStyleMoney) {
...
- (NSAttributedString *)titleForEmptyDataSet:(UIScrollView *)scrollView {
    if(_emptyDataTitle && _emptyDataTitle.length > 0 && [self dataSourceIsEmpty]) {
        NSMutableAttributedString *attString = [[NSMutableAttributedString alloc] initWithString:_emptyDataTitle];
...
- (UIImage *)imageForEmptyDataSet:(UIScrollView *)scrollView {
    ...
    if([self dataSourceIsEmpty]) {
        ...
        if(_tableStyle == ALTableStyleOrder) {
            ...
            return [UIImage imageNamed:@"yuejilu-none"];
        }
    } else {
