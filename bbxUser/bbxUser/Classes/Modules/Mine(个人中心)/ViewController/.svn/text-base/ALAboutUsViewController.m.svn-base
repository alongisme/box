...
- (void)bindAction {
    ...
    self.toolsView.multiItemBlock = ^(NSUInteger index) {
        if(index == 1) {
            ...
            [weakSelf.navigationController pushViewController:[ALInstructionsViewController new] animated:YES];
        } else if(index == 3) {
...
- (ALShadowView *)toolsView {
    if(!_toolsView) {
        _toolsView = [[ALShadowView alloc] initWithFrame:CGRectZero titleArray:@[@"去评价",@"功能介绍",@"用户协议"] contentArray:@[] rightView:YES];
