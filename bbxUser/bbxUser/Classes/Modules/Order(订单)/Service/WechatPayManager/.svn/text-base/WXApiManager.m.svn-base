...
- (void)shareRedEnvelope:(NSString *)title content:(NSString *)content icon:(NSString *)icon link:(NSString *)link scene:(int)scene {
    
    if(![WXApi isWXAppInstalled]) {
        [ALKeyWindow showHudError:@"微信未安装!"];
        return;
    }
    ...
    SDWebImageDownloader *sdwebImageDownloader = [[SDWebImageDownloader alloc] init];
    [sdwebImageDownloader downloadImageWithURL:[NSURL URLWithString:icon] options:SDWebImageDownloaderUseNSURLCache progress:nil completed:^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, BOOL finished) {
